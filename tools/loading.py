...
# from gridding import norm_cos_wgt
...
def load_data_obs(obs,Tref_start=1951,Tref_end=1980,Tblendglob_idx=False):
    ...
    if obs == 'best':
        obs_file='obs_data_25.nc' # ATTENTION: yearmean was done because I was not able to make yearmonmean
            # work (however, impact really seems negligible small)
        ds_obs=xr.open_mfdataset(dir_data+obs_file).rename({'temperature':'tas'}).sel(time=slice(1870, 2022))
...
def load_data_single_mod(gen,model,scenario,Tanglob_idx=False,Tref_all=True,Tref_start='1870-01-01',Tref_end='1900-01-01',usr_time_res="mon"):
